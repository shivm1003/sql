...
SELECT EXTRACT(MONTH FROM date_published), count(*)
FROM movie
...
-- Type your code below:
SELECT 'USA' as "country" ,count(*)
FROM movie
WHERE country LIKE '%USA%'
AND year = '2019'
UNION ALL
SELECT 'India' as "country" ,count(*)
FROM movie
...
SELECT 'total movies' AS "total_sum" , sum(total)
FROM tab;
...
+------------------+-------------------+---------------------+*/
-- Type your code below:
SELECT m.production_company, count(*),
...
WHERE (
	year = '2017' AND EXTRACT(MONTH FROM m.date_published) = '3') 
...
-- Type your code below:
SELECT 'Italian' as "language" ,sum(total_votes)
FROM movie AS m
INNER JOIN
ratings AS r
ON
...
SELECT
sum(CASE 
		WHEN n.name IS NULL THEN 1 ELSE 0 
		END) AS "name_nulls",
sum(CASE 
		WHEN n.date_of_birth IS NULL THEN 1 ELSE 0 
		END) AS "date_of_birth_nulls",
sum(CASE 
		WHEN n.height IS NULL THEN 1 ELSE 0 
		END) AS "height_nulls",
sum(CASE 
		WHEN n.known_for_movies IS NULL THEN 1 ELSE 0 
